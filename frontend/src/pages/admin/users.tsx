...
//       //check is search term is not empty befor displaying toast
//       if(searchTerm.trim() !== ""){
...
//                   </tr>):data?.users.map((user, index)=>( <tr >
...
//                         <Link to={`/dashboar/admin/user/${user?._id}`}><MdOutlineRemoveRedEye/></Link>
//                       </button>
//                     </td>
//                   </tr>))}
...
//                 <button 
//                     key={index} 
//                     className={`mx-2 pax-4 py-2 rounded-full ${currentPage===index+1 ?"bg-blue-500 text-white":"bg-gray-300"}`}
...
export default function AdminUser() {
  ...
  useEffect(() => {
    if (isError) {
      const err = error as CustomError;
      // Check if search term is not empty before displaying toast
      if (searchTerm.trim() !== '') {
        toast.error(err.data.message);
      }
    }
  ...
  return (
    <div className="w-full h-full flex flex-col">
      {/* Upper area with greeting and total user count */}
      <div className="flex-grow-0 h-1/3 bg-green-800 flex items-center justify-center text-5xl font-semibold pb-12 pt-16 px-8">
      <div>Hello {user?.name}-admin dashboard</div>
    </div>
